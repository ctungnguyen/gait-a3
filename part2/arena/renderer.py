...
class ArenaRenderer:
    ...
    def __init__(self, config: ArenaConfig, render_mode: str, theme: str = "neon"):
        try:
            import pygame
        ...
        # Presentation-only palette assignment. Physics, rewards, observations,
        # and saved-model contracts are deliberately unaffected.
        self.BG = palette.background
        self.BG_ALT = palette.background_alt
        self.GRID = palette.grid
        self.EDGE = palette.edge
        self.TEXT = palette.text
        self.MUTED = palette.muted
        self.CYAN = palette.primary
        self.BLUE = palette.secondary
        self.LIME = palette.success
        self.CORAL = palette.danger
        self.GOLD = palette.warning
        self.VIOLET = palette.accent
        pygame.init()
        pygame.font.init()
    ...
    def _draw_hud(self, core: ArenaCore, style: ControlStyle, action_label: str) -> None:
        ...
        self._text(f"{self.theme_name.upper()} PURSUIT ARENA", 31, 28, self.TEXT, self.font)
