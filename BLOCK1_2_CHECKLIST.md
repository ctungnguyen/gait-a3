...
- Block 5 reward weights are now implemented in `arena/rewards.py`; the team
  should review and justify them in the report.
- Block 6 PPO code is implemented; real TensorBoard logs and final model files
  must be generated by running the training pipeline.
